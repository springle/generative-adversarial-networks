...
def main(server, logdir, context):
    ...
    with tf.device(tf.train.replica_device_setter(
        ...
        Gz = generator(z_placeholder, z_dimensions)
        ...
        images_for_tensorboard = generator(z_placeholder, z_dimensions)
        tf.summary.image('Generated_images', images_for_tensorboard, 5)
        merged = tf.summary.merge_all()

    # Create hook for SyncReplicasOptimizer
    sync_replicas_hook = g_opt.make_session_run_hook(is_chief)

    # Create monitored training session
    with tf.train.MonitoredTrainingSession(master=server.target,
        ...
        for i in range(100000):
            ...
            if is_chief and (i % 10 == 0):
                ...
                # Print i and global_step to console
                print("Global step: %d" % sess.run(global_step))
                print("Local step: %d" % i)
